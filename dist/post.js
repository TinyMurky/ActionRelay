import { r as requireLib, a as requireUndici, g as getDefaultExportFromCjs, b as getAugmentedNamespace, c as coreExports } from './core-CW-dN4Cv.js';
...
const githubContext = githubExports.context;
...
class Logger {
    ...
    static #addHeaderToMessage(message) {
    ...
    static debug(message) {
        ...
        coreExports.debug(wrappedMessage);
    }
    static info(message) {
        const wrappedMessage = Logger.#addHeaderToMessage(message);
        coreExports.info(wrappedMessage);
    }
    static error(message) {
        if (message instanceof String || typeof message === 'string') {
            const wrappedMessage = Logger.#addHeaderToMessage(message);
            coreExports.error(wrappedMessage);
        }
        else {
            const wrappedMessage = Logger.#addHeaderToMessage(message.name);
            coreExports.error(wrappedMessage);
...
class OctokitManager {
    ...
    static getInstance(githubToken) {
        if (!this.#octokitInstance) {
            ...
            this.#octokitInstance = githubExports.getOctokit(githubToken);
        }
